...
async function generateArticle(crawlerData, memory) {
...
過去の学習：${memory || 'なし'}
以下のJSONのみ返してください：
{"title":"記事タイトル（具体的なカード名・数字を含む）","tag":"価格情報 or 環境解説 or 大会レポート or デッキ紹介","emoji":"🃏","summary":"記事の要約100文字","content":"記事本文800文字以上（具体的なカード名・価格・理由を含む）","x_post":"X投稿文140文字以内（ハッシュタグ含む）","new_insight":"記事執筆から学んだこと"}`,
      messages: [{ role: 'user', content: `【本日の収集データ】\n${dataStr}\n\n今日一番価値のある記事を1本生成してください。` }],
    }),
  });
  ...
  if (data.content?.some(b => b.type === 'tool_use')) {
    ...
    resultText = d2.content?.find(b => b.type === 'text')?.text || '';
  } else {
...
export default async function handler(req, res) {
  ...
  try {
    const [crawlerData, memory] = await Promise.all([loadTodayData(), loadMemory()]);

    if (!crawlerData.length) return res.status(200).json({ status: 'no_data' });

    const article = await generateArticle(crawlerData, memory);
    if (!article) return res.status(200).json({ status: 'failed' });
